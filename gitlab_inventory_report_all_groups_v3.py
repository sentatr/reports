...
"""
GitLab Project Inventory Report (All Groups, --group-name, Deploy Stage Aware)
------------------------------------------------------------------------------

What this script does
- Scan ALL groups in an instance (or a SINGLE group via --group-name).
- Excludes archived projects.
- Generates a CSV with the requested columns.
- Deployment Job Name(s): detects jobs whose `stage: deploy` (case-insensitive).
  If none exist -> writes "no deploy".
...
- Build Tool Used
- SQL Files Present
- Deployment Job Name(s)  (list of jobs with stage: deploy, or "no deploy")

Usage examples:
  python gitlab_inventory_report_all_groups_v3.py --base-url https://gitlab.example.com --token <PAT> --out report.csv
  python gitlab_inventory_report_all_groups_v3.py --base-url https://gitlab.example.com --token <PAT> --group-name parent/subgroup --out report.csv

Optional flags:
  --scan-scope my|all     -> 'my' scans groups visible to the token (default).
                             'all' scans all instance groups (admin token required).
...
Logs to STDOUT and gitlab_inventory_report_all_groups_v3.log
"""
...
# ------------------------- Logging Setup -------------------------
LOGGER = logging.getLogger("gitlab_inventory_report_all_groups_v3")
LOGGER.setLevel(logging.INFO)
_console = logging.StreamHandler(sys.stdout)
_console.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(message)s"))
LOGGER.addHandler(_console)
_file = logging.FileHandler("gitlab_inventory_report_all_groups_v3.log", mode="a", encoding="utf-8")
...
class ProjectRow:
    ...
    deployment_job_names: str      # comma list or "no deploy"
...
def make_session(token: str, timeout: int = 30) -> requests.Session:
    ...
    session.headers.update({
        ...
        "User-Agent": "gitlab-inventory-report/3.0"
    })
    retries = Retry(
        total=5,
        backoff_factor=0.6,
...
def detect_deployment_jobs(ci_yaml: Optional[dict]) -> List[str]:
    """Return job names whose stage is exactly 'deploy' (case-insensitive)."""
    ...
    for job_name, job_def in ci_yaml.items():
        if not isinstance(job_def, dict):
            continue
        stage = str(job_def.get("stage", "")).lower().strip()
        if stage == "deploy":
            deploy_jobs.append(job_name)
    return sorted(set(deploy_jobs))
...
def build_project_row(session: requests.Session, base_url: str, project: dict) -> ProjectRow:
    ...
    # Deployment job names: based strictly on stage: deploy
    deployment_jobs_list = detect_deployment_jobs(ci_yaml) if ci_yaml else []
    deployment_job_names = join_list(deployment_jobs_list) if deployment_jobs_list else "no deploy"
    ...
    return ProjectRow(
        ...
        deployment_job_names=deployment_job_names,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate GitLab project inventory CSV across groups, with optional single-group mode (deploy-stage aware).")
    ...
    parser.add_argument("--out", default="gitlab_projects_report_all_groups_v3.csv", help="Output CSV path")
